...
| **Git Repair Orchestrator** | `workflows/git-repair-orchestrator.json` | **Repo health ops.** POST with mode (dry-run/apply), roots, and device. Dry-run: prepares command + Gabriel acknowledgment. Apply: posts Slack approval request before any destructive action. Generates manifest.ndjson per scan. |
